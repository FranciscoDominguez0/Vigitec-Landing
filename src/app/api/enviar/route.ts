...
export async function POST(request: Request) {
  try {
    ...
    const Telefono = body['Teléfono'] || body.Telefono || body.telefono || body['TelǸfono'];
    ...
    const verifyUrl = \https://www.google.com/recaptcha/api/siteverify?secret=\&response=\\;
    ...
    // 2. Enviar correo usando Resend
    // Obligamos a usar dominguezf225@gmail.com como fallback si SMTP_DESTINATION no es valido en la capa gratuita
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            ...
            <tr>
              ...
              <td style="padding: 10px; border-bottom: 1px solid #ddd;">
                <span style="background-color: #1A1A1A; color: #fff; padding: 4px 8px; border-radius: 4px; font-size: 14px;">\</span>
              </td>
            </tr>
          </table>
    ...
    const emailResponse = await resend.emails.send({
      from: 'Vigitec Web <onboarding@resend.dev>', // Por defecto Resend usa onboarding@resend.dev en capa gratuita
      to: [destination],
